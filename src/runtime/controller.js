import { runInputCommand, commandToDirection } from "../core/engine.js";
...
export async function boot(app) {
  ...
  const controller = {
    ...
    directionOrder: [],
    repeatEligibleDirections: new Set(),
    canvasDirty: true,
    lastRenderedAssetVersion: -1,
  ...
  setupCanvasResizeTracking(controller, elements);
  ...
  window.addEventListener("keydown", (event) => {
    ...
    const directionCommand = directionCommandFromKey(key);
    const command =
      directionCommand ??
      (key === "z" ? "Z" : key === "y" ? "Y" : key === "r" ? "Restart" : null);
    if (!command) {
      return;
    }
    event.preventDefault();
    if (directionCommand) {
      noteDirectionKeyDown(controller, directionCommand, event.repeat);
      if (event.repeat) {
        if (!controller.animation) {
          const repeatCommand = consumeRepeatEligibleDirectionCommand(controller);
          if (repeatCommand) {
            handleCommand(controller, elements, repeatCommand);
          }
        }
        return;
      }
    } else {
      clearDirectionInput(controller);
    }
    handleCommand(controller, elements, command);
  });

  window.addEventListener("keyup", (event) => {
    const directionCommand = directionCommandFromKey(event.key.toLowerCase());
    if (directionCommand) {
      noteDirectionKeyUp(controller, directionCommand);
    }
  });
  ...
  const frame = () => {
    ...
    const assetsChanged = controller.lastRenderedAssetVersion !== controller.renderer.assetVersion;
    const shouldRenderCanvas =
      controller.canvasDirty ||
      assetsChanged ||
      Boolean(controller.animation) ||
      Boolean(controller.replay.active && controller.replay.playing);
    if (shouldRenderCanvas) {
      renderCanvas(controller, elements);
      controller.canvasDirty = false;
      controller.lastRenderedAssetVersion = controller.renderer.assetVersion;
    }
    ...
    if (controller.animation && performance.now() - controller.animation.startedAt > controller.animation.duration) {
      ...
      if (controller.currentState) {
        for (const block of controller.currentState.blocks) {
          block.fadeFlipH = false;
        }
      }
      controller.canvasDirty = true;
      const repeatCommand = consumeRepeatEligibleDirectionCommand(controller);
      if (repeatCommand && controller.currentState && !controller.replay.active) {
        handleCommand(controller, elements, repeatCommand);
      }
    }
 ...
 * @param {any} elements
 */
function setupCanvasResizeTracking(controller, elements) {
  const markDirty = () => {
    controller.canvasDirty = true;
  };
...
function handleCommand(controller, elements, command) {
  ...
  if (controller.animation && isDirectionCommand(command)) {
    return;
  }
  ...
  // Mirror C# PlayerMovementLogic: negate horizontal direction when camera
  // is flipped so controls stay screen-relative.
  let directionOverride = null;
  const isHorizontal = command === "L" || command === "R";
  if (isHorizontal && controller.renderer.cameraFlipH) {
    const dir = commandToDirection(command);
    if (dir) {
      directionOverride = { dx: -dir.dx, dy: dir.dy };
    }
  }

  controller.sessionInputLog.push(command);
  const turn = runInputCommand(controller.currentState, command, directionOverride);

  if (turn) {
    // Prefer C#-style turn-level animation length generated by engine.js.
    // Fallback preserves the previous behavior for older turns / replays.
    ...
    const turnDuration = Number.isFinite(turn.animLengthSeconds)
      ? Math.max(0.001, turn.animLengthSeconds) * 1000
      : hasEnterOrExit ? ENTER_LENGTH * 1000 : baseDuration;
    ...
    turn._cameraFlipHBefore = turn.fromCameraFlipH;
    turn._cameraFlipHAfter = turn.toCameraFlipH;
  }

  // C# UndoManager: Undo → Draw.FlipH = t.FromCameraFlipH; Redo → Draw.FlipH = t.ToCameraFlipH;
  if (command === "Z" && (turn?.fromCameraFlipH !== undefined || turn?._cameraFlipHBefore !== undefined)) {
    controller.renderer.cameraFlipH = turn.fromCameraFlipH ?? turn._cameraFlipHBefore;
  } else if (command === "Y" && (turn?.toCameraFlipH !== undefined || turn?._cameraFlipHAfter !== undefined)) {
    controller.renderer.cameraFlipH = turn.toCameraFlipH ?? turn._cameraFlipHAfter;
 ...
 * @returns {boolean}
 */
function isDirectionCommand(command) {
  return command === "U" || command === "D" || command === "L" || command === "R";
}

/**
 * @param {string} key
 * @returns {"U" | "D" | "L" | "R" | null}
 */
function directionCommandFromKey(key) {
 ...
 * @param {any} controller
 */
function clearDirectionInput(controller) {
  controller.pressedDirections.clear();
  controller.directionOrder = [];
  controller.repeatEligibleDirections.clear();
}

/**
 * @param {any} controller
 * @param {"U" | "D" | "L" | "R"} command
 * @param {boolean} [isRepeat]
 */
function noteDirectionKeyDown(controller, command, isRepeat = false) {
  const alreadyPressed = controller.pressedDirections.has(command);
  if (!alreadyPressed) {
    controller.pressedDirections.add(command);
    controller.directionOrder = controller.directionOrder.filter((entry) => entry !== command);
    controller.directionOrder.push(command);
  } else if (!controller.directionOrder.includes(command)) {
    controller.directionOrder.push(command);
  }
  if (isRepeat) {
    controller.repeatEligibleDirections.add(command);
    return;
  }
  controller.repeatEligibleDirections.delete(command);
}
 ...
 * @param {"U" | "D" | "L" | "R"} command
 */
function noteDirectionKeyUp(controller, command) {
  controller.pressedDirections.delete(command);
  controller.directionOrder = controller.directionOrder.filter((entry) => entry !== command);
  controller.repeatEligibleDirections.delete(command);
}
 ...
 * @returns {"U" | "D" | "L" | "R" | null}
 */
function consumeRepeatEligibleDirectionCommand(controller) {
  for (let index = controller.directionOrder.length - 1; index >= 0; index -= 1) {
    const command = controller.directionOrder[index];
    if (controller.pressedDirections.has(command) && controller.repeatEligibleDirections.has(command)) {
      controller.repeatEligibleDirections.delete(command);
      return command;
    }
  }
